...
datesSection.addEventListener("click", function (evt) {
  if (evt.target.classList.contains("delete-dates")) {
    ...
    evt.preventDefault();
    const dateOfIssue = document.querySelector("input[name='date-of-issue']")
      .value;
...
function populateCardDetails(response) {
  ...
  document.querySelector(
    "#card-div"
  ).innerHTML = `<strong>Card Details</strong>: <p>IBAN: ${response.data.IBAN}</p>`;
}

function makeAcceptBillingDetailsBtn(evt) {
  document.querySelector(
    "#accept-div"
  ).innerHTML = `<button data-toggle='tooltip' data-placement='top' title='Add billing details' class='icon accept-bi'><i class='ph-check-circle ph-xl accept-bi'></i></button>`;
  handleTooltips();
}
...
function renderDates(evt, response) {
  ...
  const dueDate = response.data.due_date.split(" ").slice(0, 4).join(" ");
  const datesRow = getTargetRow(evt);
  datesRow.firstElementChild.nextElementSibling.remove();
  datesRow.lastElementChild.remove();
  datesRow.firstElementChild.innerHTML = `<div>Date of Issue: ${date}</div>
...
function updateUIWithExtras(response) {
  ...
  if (response.data.discount) {
    document.querySelector(
      ".discount"
    ).innerText = `${response.data.discount} %`;
  }
  ...
  document.querySelector(
    "#final-amount"
  ).innerHTML = `<strong>${response.data.amount_after_extras_in_curr_of_inv} ${response.data.curr_of_inv}</strong>`;
}
